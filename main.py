import fitz  # PyMuPDF
        ...
        def translate_pdf_with_bolding(input_path, output_path, regular_font, bold_font):
            ...
            for page_num, page in enumerate(original_doc):
                ...
                # ... (The rest of this function remains the same as your original)
                        ...
                        for line in block["lines"]:
                            for span in line["spans"]:
                                ...
                                if original_text and len(original_text.strip()) >= 2:
                                    text_segments.append({'text': original_text, 'rect': fitz.Rect(span["bbox"]), 'is_bold': span['flags'] & 16, 'color': span["color"], 'size': span["size"]})
                ...
                for i, segment in enumerate(text_segments):
                    try:
                        ...
                        if isinstance(original_color, int):
                            r = ((original_color >> 16) & 255) / 255.0; g = ((original_color >> 8) & 255) / 255.0; b = (original_color & 255) / 255.0
                            normalized_color = (r, g, b)
                        elif isinstance(original_color, (list, tuple)):
                            if len(original_color) >= 3:
                                if any(c > 1.0 for c in original_color[:3]): normalized_color = tuple(c / 255.0 for c in original_color[:3])
                                else: normalized_color = tuple(original_color[:3])
                            else: normalized_color = (0, 0, 0)
                        else: normalized_color = (0, 0, 0)
                        if sum(normalized_color) > 2.7: normalized_color = (0, 0, 0)
                        text_inserted = False; original_font_size = segment['size']
                        for font_scale in [1.2, 1.0, 0.9, 0.8, 0.7, 0.6]:
                            try:
                                scaled_font_size = original_font_size * font_scale
                                result = new_page.insert_textbox(segment['rect'], translated_text, fontname=font_name_for_pdf, fontfile=font_file_to_use, fontsize=scaled_font_size, color=normalized_color, align=fitz.TEXT_ALIGN_LEFT)
                                if result >= 0: text_inserted = True; break
                            except Exception: continue
                        if not text_inserted:
                            try:
                                text_x = segment['rect'].x0; text_y = segment['rect'].y0 + (segment['rect'].height * 0.8)
                                new_page.insert_text((text_x, text_y), translated_text, fontname=font_name_for_pdf, fontfile=font_file_to_use, fontsize=original_font_size * 0.8, color=normalized_color)
